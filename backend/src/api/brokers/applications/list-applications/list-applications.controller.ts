...
export class BrokerApplicationsListController {
  ...
  async post(
    ...
    const loanAmountAboveOrBelow=body.loanAmount > avgLoanAmount ?"Loan Amount Greater Than Avg Loan Amount" :"Loan Amount Less Than Avg Loan Amount"
    const application = await this.applicationEntity.create({ ...body, status: ApplicationStatus.Submitted, brokerId: user.id });
    // console.log(application); 
    return {
      success: true,
      loanAmount,
      loanAmountAboveOrBelow
    };
  }
}
